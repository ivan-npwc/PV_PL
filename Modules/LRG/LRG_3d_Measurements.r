   ...
   #source("/home/ivan/GIT_HUB/PV_PL/Modules/LRG/LRG_3d_Measurements.r")
		...
		library(plotly)
		library(RANN)
		source("/home/ivan/GIT_HUB/PV_PL/Modules/LRG/FUNCTIONS_SEAL_3D_MEASUREMENTS.r")# upload function to processing
	...
	#	 labelInput = "/mnt/adata8tb/PV_DB/2023_H0052A_OPP/20230518_104633/20230518_104633_MINI2_20m"
        
		# Set species and extract date from directory name
		Species <- "LRG"
		date1 <- substr(basename(labelInput), 1, 15)
		MeshDir = file.path(labelInput, "Predict","MeshCrop")
		MeshSaveDir = file.path(labelInput, "Predict","MeshCrop_proceed");#unlink(MeshSaveDir,recursive=T);dir.create(MeshSaveDir)
	
		MeshLst=list.files(MeshDir, full.names=T)
		saveCSVPth= file.path(labelInput, "Predict",paste0(date1,"_sealVolume.csv"))
		saveKMLpth= file.path(labelInput, "Predict",paste0(date1,"_sealVolume.kml"))
		presence = list.files(MeshSaveDir)
		skipp =   basename(presence) %in%  basename(MeshLst) 
		print(paste0("SKIP     ", presence))
		############################################################
		fin=NULL
		for (y in 1:length(MeshLst)){ #
		 # y=8
		  ...
		  slpth=  MeshLst[y]
		#  if (basename(slpth) %in% presence == T) {next}
		  nme=basename(slpth)
		  saveMeshPth=file.path(MeshSaveDir,nme)
		  flsize= file.size(MeshLst[y])
		 # if (flsize < 90000) {unlink(slpth)} else {
		  sl = vcgImport(slpth)
		 #vcgPlyWrite(sl3, "mesh.ply")
		  
		  
		  sl2= mesh_reconstract(sl)
		  sl3 = close_small_holes(sl2)
		  sl4 =frontalis_slice_filter(sl3)
		   if (is.null(sl4)) next 
		  sl5=axial_slice_filter(sl4)
		  sl6=saggital_slice_filter(sl5)
#vcgPlyWrite(sl6, "mesh.ply")
		...
		##############################
			...
			sl10 = saggital_slice_filter(sl9)
			 sl12= mesh_reconstract(sl10)
			 ###
			 orientation=determine_mesh_orientation(sl12)
			 iter=0;max_iter=10
			   while(orientation == "midi up" && iter < max_iter) {
			iter <- iter + 1
			 message("Attemt ", iter, ": mesh overturn")
		...
		determine_mesh_orientation(sl12)
			#### 
			 
			 
			#vcgPlyWrite(sl8, "mesh.ply") 
	################################################ cut 1
			bottom = Get_bottom(sl12, alt_correct=0.017) 
			#vcgPlyWrite(bottom, "bottom.ply") 
         #  mergseal =merg_seal_ground(sl12, bottom)

		 	trimmed_seal = trim_seal_by_ground (seal_mesh=sl12, ground_mesh=bottom, above = T)	

		
			#vcgPlyWrite(sl18, "mesh.ply") 
		...
		################################################################################	cut 2
				...
				sl17 = close_small_holes(sl16)
				sl18 = mesh_reconstract(sl17)
				
				#vcgPlyWrite(seal_mesh, "mesh.ply") 
				altmsh =median(t(sl18$vb)[,3])
				if (altmsh<0){next}
				
			seal_mesh = clip_mesh_bottom(sl18,percent=12)
			ground_mesh = Get_bottom(trimmed_seal, alt_correct=0)
	        	sl_m = merg_seal_ground(sl18, ground_mesh)
				
			#vcgPlyWrite(sl_m, "sl_m.ply") 
		
	
   #         inside <- Rvcg::vcgClost(ground_mesh, trimmed_seal, sign = TRUE)$quality > 0
   #         trimmed_ground_pts <- vert2points(ground_mesh)[inside, ]
   #         trimmed_ground_mesh <- vcgBallPivoting(trimmed_ground_pts,radius=0.02)
      #vcgPlyWrite(trimmed_ground_mesh, "mesh.ply") 
			#sl_m=mergeMeshes(seal_mesh,trimmed_ground_mesh)
#			sl_m=merg_seal_ground(seal_mesh,trimmed_ground_mesh)
      #vcgPlyWrite(sl_m, "mesh.ply") 
#			sl_m1 = mesh_reconstract(sl_m)
#			sl_m2 = vcgSmooth(sl_m1,type = "laplace", iteration = 300)
			 #vcgPlyWrite(sl_m2, "mesh.ply") 
