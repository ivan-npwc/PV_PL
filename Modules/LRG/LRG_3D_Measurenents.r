	...
	#	 library(KernSmooth)
     ...
     #  labelInput <- "D:\\PV_DB\\2023_H0052A_OPP\\20230615_103330\\20230615_103330_MAVIC2PRO_40m"
		...
		MeshSaveDir = paste0(labelInput, "\\Predict\\MeshCrop_proceed") ; dir.create(MeshSaveDir,showWarnings = F)
	  
	   errdir  = paste0(labelInput, "\\Predict\\MeshCrop_error") 
	
		MeshLst=list.files(MeshDir, full.names=T)
		if (file.exists(MeshLst[1])==T){
		
		source("C:\\Users\\usato\\SSL_DB\\PV_PL\\Modules\\LRG\\FUNCTIONS_SEAL_3D_MEASUREMENTS.r")# upload function to processing
		...
		# y=1
			  slpth=  MeshLst[y]
			#  if (file.exists(slpth)==F) {next}
			  nme=basename(slpth)
			  saveMeshPth=paste0(MeshSaveDir,"\\",nme)
		if (file.exists(saveMeshPth)) {next} 	  
		...
		if (vcgArea(sl) < 0.7 || vcgArea(sl) > 1.8) { dir.create(errdir,showWarnings = F); file.copy(slpth, errdir); unlink(slpth); next}
	 
	    
	   
		  sl2 = close_small_holes(sl)        
		  sl4=frontalis_slice_filter(sl2)    
		  ...
		  altrng
		  if (altrng < 0.21){ message("Mesh is  flat   ", y)
           dir.create(errdir,showWarnings = F); file.copy(slpth, errdir); unlink(slpth)
		  next}
		...
		##############################
			...
			sl12= mesh_reconstract(sl11)
			 ...
			 #vcgPlyWrite(trimmed_seal,  "mesh.ply")
	################################################ cut 1
	...
	################################################################################	cut 2
			...
			altmsh
			if (altmsh<0){ dir.create(errdir,showWarnings = F); file.copy(slpth, errdir); unlink(slpth); next}
			
			#seal_mesh = clip_mesh_bottom(sl16,percent=12)
			ground_mesh = Get_bottom(seal_mesh, alt_correct=0)
	        #vcgPlyWrite(ground_mesh,  "mesh.ply")          
...
for (i in 1:30){
 sl_m1 = mesh_reconstract(sl_m1)
 fixFace = fixNormalsCorrected(sl_m1)
 sl_m1=fixFace[[1]]
    if (is_watertight(sl_m1)==F){ sl_m1=close_small_holes(sl_m1)}
    if (fixFace[[2]]==0) {break}
 } 
 
			
               
		#	sl_m1 = mesh_reconstract(sl_m)
			#   vcgPlyWrite(sl_m1,  "check.ply")
