...
if (file.exists(kmlOPP) == F) {stop("No kml OPP found")}  	 
...
######################################
 ...
 #for (i in 1:Batch){
      ...
      path=selectRow$imgName
	  CheckExists = paste0(SaveDir, "\\",basename(path))
    ...
    #  if(file.exists(CheckExists)==F & file.exists(path)) {                                                                   
      ...
      #    dlimg = paste0( Downlevel$leftName)  
       ...
       #   drimg=paste0( Downlevel$rightName) 
