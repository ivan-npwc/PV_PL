...
seal_cut=function(PolPth,site_3d_gps_pth, seal_num = 1){
...
singlePol <- poly_sf$geometry[seal_num]
...
# Инициализация Python-модулей
o3d <- import("open3d")
np <- import("numpy")
  # pymeshlab <- import("pymeshlab")

# Функция для конвертации R-mesh3d в Open3D mesh
r_to_o3d <- function(r_mesh) {
  vertices <- t(r_mesh$vb[1:3,])
  faces <- t(r_mesh$it - 1)  # Open3D использует 0-индексацию
...
# Функция для конвертации Open3D mesh в R-mesh3d
o3d_to_r <- function(o3d_mesh) {
  vertices <- py_to_r(np$asarray(o3d_mesh$vertices))
  faces <- py_to_r(np$asarray(o3d_mesh$triangles)) + 1  # Возвращаем 1-индексацию
...
mrg5 = NULL
  ...
  sl=seal_cut(PolPth,site_3d_gps_pth,seal_num = 13)
