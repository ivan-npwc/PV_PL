...
draft=function(){
######################################################################
   meshRestorPCA=function(){


	      sl_m5=sl_m4
		  MeshOrigOrientation = pca_to_orig(meshBase=sl6,meshPCA=sl_m5)
		
		   sl_m6=MeshOrigOrientation
			
			for (i in 1:100){
			sl_m6=mesh_reconstract(sl_m6)
			sl_m6=close_small_holes(sl_m6)	
			if(is_watertight(sl_m6)==T){break}
		     }
		
		#pl(sl_m6); pl(sl,2,0.4)
		
		spine= get_spine(sl_m6)
		spine_length <- sum(sqrt(rowSums(diff(spine)^2)))
		
		chest = get_chest(sl_m6)
		chest_length <- sum(sqrt(rowSums(diff(chest)^2)))
		
		pl(sl_m6);points3d(chest,col=2,size=10);points3d(spine,col=3,size=10);
		
		
		if(is_watertight(sl_m6)==F){ message("Error to create  watertight mesh", y);  next}
		weightMesh	= vcgVolume(sl_m6)
			
	}
##############################################get coords
		
	 #	pl(sl_m5);pl(sl,2,0.4)

	    #pnts = PointySize(sl_m, initial_points=9000, length.out=100, type="km", depth	=0.01) ## mc pd km
