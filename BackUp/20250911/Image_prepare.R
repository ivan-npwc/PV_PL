...
##############################################################################################
   ...
   PointsPorderImage=rbind(CombinationOne,CombinationTo,CombinationThry,CombinationFor)
    ...
    PointsImg <- SpatialPointsDataFrame(coords = coords,
                                     data = data, 
                                     proj4string = crs)
	PointsImg =	 st_as_sf(PointsImg)							 
		 PredPol = st_read(PredPolPth)
		 PredPol <- PredPol %>% st_transform(crs)
         PredPol <- st_buffer(PredPol, dist = 4)
		 

		 Index= st_intersects(PointsImg, PredPol, sparse = F)
	     ImgInALL=NULL
	for (i in 1:length (Index[1,])){
	     polindex=Index[,i]
		 PointsImg$Index=polindex
		 ImgInSub = PointsImg$imgName[PointsImg$Index==T]
		 ImgInALL=c(ImgInALL,ImgInSub)
		 }
		  ImgIn =	unique(ImgInALL)
######################################
