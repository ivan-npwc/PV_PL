...
##################################################################
# Helper function to validate mesh
validate_mesh <- function(mesh, operation_name = "unknown") {
  if (is.null(mesh)) {
    message("Mesh is NULL after: ", operation_name)
    return(FALSE)
  }
  
  if (!inherits(mesh, "mesh3d")) {
   message("Object is not a mesh3d after: ", operation_name)
    return(FALSE)
  }
  
  if (is.null(mesh$it) || ncol(mesh$it) == 0) {
    message("Mesh has no triangular faces after: ", operation_name)
    return(FALSE)
  }
  
  if (ncol(mesh$vb) == 0) {
    message("Mesh has no vertices after: ", operation_name)
    return(FALSE)
  }
  
  # Check for degenerate triangles
  if (any(apply(mesh$it, 2, function(x) length(unique(x)) < 3))) {
    message("Mesh has degenerate triangles after: ", operation_name)
    return(FALSE)
  }
...
mesh_reconstract=function(mesh,ball_radius=0.02,num_samples=10000){
  ...
  reconstract <- vcgBallPivoting(smpls, radius = ball_radius)
   ...
   vec= t(reconstract$vb)[,3]
   altrng=diff(range(vec))
   if (altrng < 0.15) {break; return (NULL)}
