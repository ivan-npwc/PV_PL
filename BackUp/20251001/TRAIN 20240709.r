...
##########################################################################
######################################
source("C:\\Users\\usato\\SSL_DB\\PV_PL\\Modules\\UNET\\UnetVGG16CreateLRG.r") #UnetVGG16Create
...
##############################################################
# САМАЯ ПРОСТАЯ И НАДЕЖНАЯ версия
dice_coef <- custom_metric("dice_coef", function(y_true, y_pred, smooth = 1.0) {
  # Простое вычисление без сложного reshape
  intersection <- k_sum(y_true * y_pred)
  (2 * intersection + smooth) / (k_sum(y_true) + k_sum(y_pred) + smooth)
})
...
#	 if (runif(1) > 0.5) {
#    angle <- runif(1, -30, 30)
#    img <- tf$image$rot90(img, k = as.integer(angle / 90))
#    mask <- tf$image$rot90(mask, k = as.integer(angle / 90))
 # }
#		dataset <- dataset %>% 
